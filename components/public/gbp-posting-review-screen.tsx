...
import {
  ...
  type PublicGbpPostingContentResponse,
...
export const GbpPostingReviewScreen = ({
  token,
}: GbpPostingReviewScreenProps) => {
  const toast = useAppToast();
  const attachmentInputRef = useRef<HTMLInputElement | null>(null);
  const commentEditorRef = useRef<HTMLDivElement | null>(null);
  const commentImageInputRef = useRef<HTMLInputElement | null>(null);
  const postingImageInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const [posting, setPosting] = useState<PublicGbpPosting>(emptyPosting);
  const [contentData, setContentData] =
    useState<PublicGbpPostingContentResponse | null>(null);
  ...
  const pageTitle = useMemo(
    () => status?.postingTitle || posting.keyword || "GBP Posting Review",
    [posting.keyword, status?.postingTitle],
  );
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      ...
      try {
        const response = await gbpPostingReviewsApi.getPublicContent(
          token,
          reviewSessionToken,
        );
        ...
        setPosting({
          ...emptyPosting,
          ...response.posting,
          images: Array.isArray(response.posting.images)
            ? response.posting.images
            : [],
          postContent: response.posting.postContent ?? "",
        });
      } catch (loadError) {
        if (!isMounted) {
          return;
  ...
  const handleSendOtp = async () => {
    ...
    try {
      await gbpPostingReviewsApi.sendOtp(token, { email, fullName });
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await gbpPostingReviewsApi.verifyOtp(token, {
        email,
        otp,
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await gbpPostingReviewsApi.savePublicContent(
        token,
        reviewSessionToken,
        {
      ...
      toast.danger("Failed to save posting.", {
  ...
  const handlePostingImageUpload = async (files: FileList | null) => {
    if (!files?.length) {
      return;
    }
  ...
  const handleRemovePostingImage = (index: number) => {
    setPosting((current) => ({
      ...current,
  ...
  const handleClearPostingImages = () => {
    setPosting((current) => ({
      ...current,
      images: [],
    }));
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await gbpPostingReviewsApi.addPublicComment(
        token,
        reviewSessionToken,
        message,
      );
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    try {
      await gbpPostingReviewsApi.deletePublicComment(
        token,
        reviewSessionToken,
        commentId,
      );
  ...
  const primaryImage = posting.images[0] ?? "";
        ...
        <header className="rounded-lg border border-default-200 bg-white p-4">
          ...
          {posting.contentType ? (
            <p className="mt-1 text-xs text-[#6B7280]">
              {posting.contentType}
              {posting.audience ? ` · ${posting.audience}` : ""}
            </p>
          ) : null}
        </header>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
          <section className="space-y-4 rounded-lg border border-default-200 bg-white p-4">
            <div>
              ...
              <Select
                aria-label="Button"
                id="review-button-type"
                placeholder="Choose a button"
                selectedKeys={posting.buttonType ? [posting.buttonType] : []}
                size="sm"
                variant="bordered"
                onSelectionChange={(keys) => {
                  ...
                  setPosting((current) => ({
                    ...current,
                    buttonType: selectedKey ? String(selectedKey) : null,
                  }));
                }}
              >
            ...
            <div>
              <FieldLabel htmlFor="review-post-content">
                Post Content
              </FieldLabel>
              <Textarea
                id="review-post-content"
                minRows={14}
                value={posting.postContent ?? ""}
                variant="bordered"
                onValueChange={(value) => {
                  setPosting((current) => ({ ...current, postContent: value }));
                }}
              />
            </div>
          </section>
          ...
          <aside className="space-y-4 lg:sticky lg:top-4 lg:self-start">
            ...
            <section className="space-y-4 rounded-lg border border-default-200 bg-white p-4">
              <h2 className="text-sm font-semibold text-[#111827]">Images</h2>
              <div className="w-full rounded-xl border border-default-200 p-2">
                <button
                  ...
                  onClick={() => {
                    if (primaryImage) {
                      ...
                      postingImageInputRef.current?.click();
                    }
                  }}
                >
                  {primaryImage ? (
                ...
                {posting.images.length > 1 ? (
                  <div className="mt-2 grid grid-cols-4 gap-1">
                    {posting.images.slice(1).map((image, index) => (
                      <div
                        key={`${index + 1}-${image.slice(0, 24)}`}
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={(event) => {
                            ...
                            handleRemovePostingImage(index + 1);
                          }}
                        >
                          <X size={10} />
                        </button>
                ...
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <Button
                    isIconOnly
                    isDisabled={posting.images.length === 0}
                    radius="full"
                    size="sm"
                    variant="flat"
                    onPress={handleClearPostingImages}
                  >
                    <Trash2 size={14} />
                  </Button>
                </div>
                <input
                  ref={postingImageInputRef}
                  multiple
                  accept="image/*"
                  className="hidden"
                  type="file"
                  onChange={(event) => {
                    void handlePostingImageUpload(event.target.files);
                    event.target.value = "";
                  }}
                />
              </div>
