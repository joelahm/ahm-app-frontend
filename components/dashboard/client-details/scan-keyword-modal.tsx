...
const ScanCoverageMapPreview = ({
  isLoading,
  preview,
}: {
  isLoading: boolean;
  preview: ScanCoveragePreview;
}) => {
  return (
    <div className="lg:sticky lg:top-0">
      <div className="mb-2 flex items-center justify-between gap-3">
        <p className={labelClassName}>Pin preview</p>
...
export const ScanKeywordModal = ({
  ...
  const hasConnectedCoordinates =
    typeof gbpDetails?.latitude === "number" &&
    typeof gbpDetails?.longitude === "number";
  const scheduleDateValue = useMemo(
    () => toDateValue(scheduleDate),
    [scheduleDate],
  );
  const coveragePreview = useMemo<ScanCoveragePreview>(() => {
    const label = gbpDetails?.businessName ?? gbpDetails?.businessLocation;
  ...
  useEffect(() => {
    ...
    onPreviewChange?.(coveragePreview);
  }, [coveragePreview, isOpen, onPreviewChange]);
                  ...
                  <CardBody className="space-y-2 p-4">
                    {isLoadingGbp ? (
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-default-500">
                          Connect GBP in the GBP page first before running a
                          local rankings scan.
                        </p>
                      </div>
                    )}
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        placeholder={
                          field.value ? "Select value" : "Select size"
                        }
                        radius="sm"
