...
interface TaskPanelAttachmentsProps {
...
export const TaskPanelAttachments = ({ taskId }: TaskPanelAttachmentsProps) => {
  ...
  const handleUpload = async (file: File) => {
    if (!session?.accessToken || isUploading) return;
  ...
  const handleDelete = async () => {
    if (!attachmentToDelete || !session?.accessToken) return;

    try {
      const accessToken = await getValidAccessToken();

      await clientsApi.deleteTaskAttachment(accessToken, attachmentToDelete.id);
